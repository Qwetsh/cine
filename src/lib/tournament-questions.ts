...
async function generateStreetQuestions(
  ...
  const movies = await discoverMoviesByTheme(theme.kind, theme.value)
...
async function generateGeneralQuestions(count: number): Promise<QuizQuestion[]> {
  const movies = await discoverMoviesByTheme('general', null)
  if (movies.length === 0) return []
...
export async function generateTournamentQuestions(
  ...
  const streetQuestionsPromises = streetThemes.map(theme =>
    generateStreetQuestions(theme, tilesPerStreet + 2) // extra buffer
  )

  // General questions for intro tiles
  const introCount = 4 + 2 // 2 intro tiles per side + 1 merge tile per side
  const generalPromise = generateGeneralQuestions(introCount + 10) // +10 for fight
